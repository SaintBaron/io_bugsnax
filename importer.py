...
from .parser import parse_xcache_file
...
# Per-image transparency cache (keyed by datablock name) so a texture
# reused across many materials is only inspected once.
_ALPHA_CACHE: dict = {}


def _image_has_transparency(img) -> bool:
    """Return True only if the image genuinely contains sub-1.0 alpha.

    Mirror of the .xcache importer's check, as a standalone function.
    Most diffuse textures are fully opaque; only foliage / cutout
    textures (common in daecache: leaves, bushes, palms) carry real
    transparency. We must not blend the opaque ones or every textured
    surface goes see-through.

    Never index img.pixels element-by-element (each access is a
    separate Python->C call). Read the whole buffer in one bulk call
    and slice the alpha channel. Cheapest checks first (no pixel read):
      1. None / 1x1 placeholder              -> opaque
      2. No alpha channel (depth 8/24, <4ch) -> opaque
    Any error -> assume opaque (safe default keeps the material solid).
    """
    try:
        if img is None:
            return False
...
def _build_material(
    ...
    if len(textures) >= 1:
        ...
        if diff is not None:
            ...
            # sub-1.0 alpha. daecache foliage (leaves, bushes, palms)
            # encodes cutout transparency in the diffuse alpha; opaque
            # textures are left fully solid so they don't go see-through.
            # Same approach as the .xcache importer: CLIP (alpha
            # threshold), not BLEND, so only the genuinely transparent
            # pixels are discarded and the rest stays solid and clean.
...
# ============================================================================
# .xcache  (SEMS skinned-character cache)
# Consolidated from the former xcache_importer.py. This is the frozen,
# reverse-engineered SEMS logic (validated against a large Bugsnax dev
...
def import_xcache(
    ...
    **_,
    ...
    if import_armature and frame_nodes:
        ...
        # was applied here, which left imported characters facing -Y.
        # Dropped so models face +Y (forward) as requested.
...
class _ImportState:
    ...
    def parse_material(self, node, base_dir):
        ...
        if self.import_textures and (original_tex_name or convention_tex_candidates):
            ...
            if img is not None:
                ...
                # channel (leaf cutouts, eye highlights, etc.) but MOST
                # are fully opaque (alpha == 1 everywhere).
                #
                # We must only switch a material into a transparent
                # blend mode when its texture genuinely contains
                # sub-1.0 alpha. Otherwise every textured material
                # becomes see-through. (The old code used 'HASHED' for
                # all of them — that happened to look fine on opaque
                # textures because nothing gets discarded at alpha==1,
                # but it made the genuinely-transparent ones grainy.
                # Switching everything to 'BLEND' instead made the
                ...
                if getattr(self, "use_diffuse_alpha", False):
                    try:
                        ...
                        if has_alpha:
                            ...
                            # Use 'CLIP' (alpha threshold), NOT 'BLEND'.
                            #
                            # These are cutout-style textures: mostly
                            # opaque, with specific transparent regions.
                            # BLEND composites the WHOLE surface through
                            # alpha — so any pixels whose alpha is a bit
                            # under 1.0 (and the mesh interior, via back
                            # faces) make the entire part look
                            # see-through. CLIP instead keeps every
                            # pixel at/above the threshold fully solid
                            # and only discards the genuinely
                            # transparent pixels below it. Result: only
                            ...
                            try:
                                ...
                                pass
                            # show_transparent_back is a BLEND concept and
                            # made closed meshes look hollow; ensure it's
                            # off for the clip path.
                            try:
                                mat.show_transparent_back = False
                            except Exception:
                                pass
    ...
    def _image_has_transparency(self, img) -> bool:
        """Return True only if the image genuinely contains sub-1.0 alpha.

        Most Bugsnax diffuse textures are fully opaque (alpha == 1
        everywhere); only a handful carry real transparency. We must not
        blend the opaque ones or every textured surface goes see-through.

        Performance: NEVER index img.pixels element-by-element — each
        access is a separate Python→C call and is ruinously slow on big
        textures. We read the whole pixel buffer in one bulk call
        (foreach_get into a numpy/array buffer) and slice the alpha
        channel. Results are cached per image datablock so a texture
        reused across many materials is only inspected once.

        Cheapest checks first (no pixel read at all):
          1. None / 1×1 placeholder            → opaque
          2. No alpha channel (depth 8/24, <4 ch) → opaque
        Any error → assume opaque (safe default keeps material solid).
        """
        try:
            if img is None:
                return False

            # Per-image cache (keyed by datablock name) — same texture
            # across many materials is inspected only once.
            ...
            def _decide():
                ...
                # Bulk-read the entire pixel buffer in ONE call.
                buf = None
                try:
                    import numpy as np
                    ...
                    # Fallback without numpy: still ONE bulk copy via
