...
Each format's code lives in one module per role: parser.py (binary
read/write), importer.py (cache -> Blender), exporter.py (Blender ->
cache).
"""
...
class ExportXcache(bpy.types.Operator, ExportHelper):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Animation Range", icon="TIME")
